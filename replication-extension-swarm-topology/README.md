...
baseline. We then vary c₁, the weight of each agent's personal-best memory.

**Result 1: with default swarm (c₁=1.5), the topology effect disappears.**

Mean cooperation over the (T, S) grid:

| Network | Imitation | Swarm c₁=1.5 | Swarm c₁=0 |
|---|---|---|---|
...
![](topology_swarm_vs_imitation.png)

**Result 2: removing memory (c₁=0) restores the effect, and it becomes stronger than under
...
**Result 3: the effect declines steadily as c₁ increases, while stable PD cooperation
vanishes already at c₁=0.25.** A velocity test shows agents come to rest only when c₁=0.
...
paper. With c₁>0, swarm's PD values (~0.12) are threshold flicker, not cooperation. The
