...
def run(network_kind, n, generations, R, S, T, P, seed, m=4, avg_degree=8):
    """
    ...
    for _ in range(generations):
        payoffs = compute_payoffs(graph, programs, R, S, T, P)
        programs = {
...
def run_batch(network_kind, n, generations, R, S, T, P, num_realizations, seed, m=4, avg_degree=8):
    """`num_realizations` independent runs, seeded seed, seed+1, ... like ../main.py's run_batch()."""
    return [
        run(network_kind, n, generations, R, S, T, P, seed + r, m, avg_degree)
