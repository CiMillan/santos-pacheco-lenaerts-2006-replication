...
with the paper's probability. $\lambda=1$ is exactly the paper's rule (checked: same seed, identical
run). Smaller $\lambda$ means longer memory. Two controls were added to test hypotheses:
- **Peak trace**: jumps up to any new high payoff and fades down at rate $\lambda$. It holds on to
...
Everything else is the same as [swarm topology](../extension-2-swarm-topology/):
...
$\lambda$, and long memory nudges heterogeneous networks slightly up. This is the opposite of what we
predicted.
...
Mean cooperation over the $(T, S)$ grid at $\lambda=0.05$ ($\lambda=1$ is the paper's rule):

| Network | $\lambda=1$ (paper) | Average trace | Peak trace | Strategy memory |
|---|---|---|---|---|
