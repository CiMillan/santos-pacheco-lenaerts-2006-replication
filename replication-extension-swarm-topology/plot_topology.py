...
def plot_topology_grid(rows, T_values, S_values, suptitle=""):
    """
    ...
    for i, (label, (imitation, swarm)) in enumerate(rows.items()):
        plot_heatmap(imitation, T_values, S_values, ax=axes[i][0], title=f"{label} -- imitation")
        plot_heatmap(swarm, T_values, S_values, ax=axes[i][1], title=f"{label} -- swarm")
        ...
        axes[i][2].set_title(f"{label} -- swarm minus imitation")
