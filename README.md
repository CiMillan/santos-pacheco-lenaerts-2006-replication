...
Each extension changes one part of the base model and holds the rest fixed. Read them in order:
the folders in [`replications/`](replications/) are numbered, and later ones reuse code or results
from earlier ones.

| # | Variable under test | Held fixed |
|---|---|---|
| 1 | [**Swarm learning**](replications/extension-1-swarm-learning/): the update rule, the paper's imitation vs. particle-swarm learning (PSO) | one Prisoner's Dilemma ($T=1.2$, $S=-0.1$) on the Barabási–Albert network |
| 2 | [**Swarm topology**](replications/extension-2-swarm-topology/): the network type, all four; then PSO's memory weight $c_1$ | PSO update rule, $(T, S)$ grid |
| 3 | [**Stigmergic imitation**](replications/extension-3-stigmergy/): what agents copy from, a neighbour's current payoff vs. a fading trace left at its node (memory $\lambda$) | imitation rule, four networks, $(T, S)$ grid |
| 4 | [**Open-source game theory**](replications/extension-4-open-source/): what agents hold, a plain strategy vs. a program that reads the other's code; then a code-reading cost | imitation rule, four networks, $(T, S)$ grid |
...
| 1 | [`update_rule.py`](update_rule.py) → [`swarm_update_rule.py`](replications/extension-1-swarm-learning/swarm_update_rule.py); [`main.py`](main.py) → [`main_swarm.py`](replications/extension-1-swarm-learning/main_swarm.py) | [`compare_imitation_vs_swarm.py`](replications/extension-1-swarm-learning/compare_imitation_vs_swarm.py), [`plot_comparison.py`](replications/extension-1-swarm-learning/plot_comparison.py) |
| 2 | [`sweep.py`](sweep.py) → [`sweep_swarm.py`](replications/extension-2-swarm-topology/sweep_swarm.py); [`plot.py`](plot.py) → [`plot_topology.py`](replications/extension-2-swarm-topology/plot_topology.py) | [`topology_experiment.py`](replications/extension-2-swarm-topology/topology_experiment.py), [`c1_zero_experiment.py`](replications/extension-2-swarm-topology/c1_zero_experiment.py), [`c1_sweep.py`](replications/extension-2-swarm-topology/c1_sweep.py), [`velocity_test.py`](replications/extension-2-swarm-topology/velocity_test.py), [`plot_c1_curve.py`](replications/extension-2-swarm-topology/plot_c1_curve.py) |
| 3 | [`update_rule.py`](update_rule.py) → [`stigmergy_update_rule.py`](replications/extension-3-stigmergy/stigmergy_update_rule.py); [`main.py`](main.py) → [`main_stigmergy.py`](replications/extension-3-stigmergy/main_stigmergy.py); [`sweep.py`](sweep.py) → [`sweep_stigmergy.py`](replications/extension-3-stigmergy/sweep_stigmergy.py) | [`lam_experiment.py`](replications/extension-3-stigmergy/lam_experiment.py), [`plot_lam_curve.py`](replications/extension-3-stigmergy/plot_lam_curve.py) |
| 4 | [`main.py`](main.py) → [`main_os.py`](replications/extension-4-open-source/main_os.py); [`payoff.py`](payoff.py) → [`payoff_os.py`](replications/extension-4-open-source/payoff_os.py); [`sweep.py`](sweep.py) → [`sweep_os.py`](replications/extension-4-open-source/sweep_os.py) | [`programs.py`](replications/extension-4-open-source/programs.py), [`open_source_experiment.py`](replications/extension-4-open-source/open_source_experiment.py), [`cost_experiment.py`](replications/extension-4-open-source/cost_experiment.py) |
| 5 | [`main.py`](main.py) → [`main_bots.py`](replications/extension-5-network-engineering/main_bots.py) | [`bots.py`](replications/extension-5-network-engineering/bots.py) *(WIP: sweep, experiment, plot to come)* |
...
| 1 | [**Swarm learning**](replications/extension-1-swarm-learning/): particle-swarm (PSO) learning instead of the paper's imitation; PD on the BA network | **Replacing imitation with PSO cuts cooperation from 0.90 to 0.33,** because each agent's personal best keeps pulling it back to an old defection windfall. |
| 2 | [**Swarm topology**](replications/extension-2-swarm-topology/): swarm learning on all four networks; vary memory weight $c_1$ | **The paper's topology effect survives only for learners without self-memory:** default PSO flattens every network to ≈0.58, while PSO with no personal memory ($c_1=0$) makes the effect stronger than the paper's (BA 0.88 vs. 0.65). |
| 3 | [**Stigmergic imitation**](replications/extension-3-stigmergy/): agents read a fading payoff trace left at each node instead of the neighbour's current payoff (idea from SwarmWorld) | **A remembered payoff is harmless; a remembered strategy weakens network reciprocity** (BA–complete gap 0.16 → 0.12), but far less than PSO's private, never-fading memory, which erased it. |
| 4 | [**Open-source game theory**](replications/extension-4-open-source/): agents hold programs that read each other's code; then add a code-reading cost | **When agents can read each other's code for free, the network stops mattering** (≈0.97–1.00 everywhere); the paper's effect returns only once code-reading costs 0.5 or more, and is back to the paper-rule values (within 0.01) at 1.0. |
| 5 | [**Network-engineering bots**](replications/extension-5-network-engineering/): bots that rewire agents' ties, from Shirado & Christakis (2020) | **Work in progress:** not run yet. |
